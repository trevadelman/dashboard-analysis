...
async function loadBotActions() {
    ...
    try {
        const actions = await (await fetch('/api/bot/actions?limit=20')).json();
        if (!Array.isArray(actions) || actions.length === 0) {
            container.innerHTML = `<div class="text-center text-base-content/50 py-4 text-sm">
                <i class="bi bi-robot text-2xl mb-1 block"></i>
        ...
        const rows = actions.map(a => {
            ...
            const sym   = a.symbol || '—';
            const result = (a.result || '').substring(0, 80);
            return `<tr class="hover text-xs">
                <td class="font-mono text-xs text-base-content/50">${time}</td>
                <td><span class="badge ${cfg.badge} badge-sm gap-1"><i class="bi ${cfg.icon}"></i>${cfg.label}</span></td>
                <td class="font-bold">${sym}</td>
                <td class="text-base-content/70 max-w-xs truncate">${result}</td>
            </tr>`;
        }).join('');
