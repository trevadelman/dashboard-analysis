...
The bot must refuse to trade outside market hours, when the account is down more than
the daily loss limit, or when a per-symbol cooldown is active. These are hard stops,
not soft warnings.
...
- [ ] **`scheduler.py`** — APScheduler wrapper; starts with Flask app; registers all jobs;
      graceful shutdown on app exit; exposes `get_scheduler_status()` for the dashboard
- [ ] **`strategies/auto_manager.py`** — core autonomous loop; all bot decisions flow here;
      reads `bot_state.json`, acts, writes results back
- [ ] **`data/bot_state.json`** — persistent state file:
...
- [ ] **`data/bot_actions.json`** — append-only action log (replaces notifications);
...
- [ ] **Hourly position review job** — fires every hour during market hours;
      calls `PositionReviewer` for each open swing position; logs verdict to `bot_actions.json`
- [ ] **Daily position review job** — fires at 9:35 ET each trading day;
      reviews all long (daily) positions
- [ ] **Auto TRAIL_STOP** — if verdict is TRAIL_STOP and `suggested_stop` differs from
      `current_stop` by > $0.01, call `bot.adjust_orders()`; log action
- [ ] **Auto EXIT** — if verdict is EXIT, call `bot.close_position()`; log action
- [ ] **Exit detection poller** — runs every 5 min; compares `bot_state.last_positions`
...
- [ ] **Cooldown guard** — don't act on the same symbol more than once per review cycle;
...
- [ ] **15-min scan job** — fires every 15 min during market hours;
      scans short-timeframe watchlist (configurable in `.env` or `config.py`)
- [ ] **Hourly scan job** — fires every hour during market hours; scans swing watchlist
- [ ] **Daily scan job** — fires at 9:40 ET; scans long-timeframe watchlist
- [ ] **Signal deduplication** — compare new signals to `bot_state.last_signals[timeframe]`;
      only act on signals whose symbol was NOT in the previous cycle's signal list
- [ ] **`can_trade()` gate** — already implemented; enforces max positions, no duplicate
      symbols, no pending orders; called before every auto-entry
- [ ] **Auto-execute bracket order** — when new signal passes all gates, call the same
      bracket order logic as the manual UI (`execute_trade` endpoint logic);
      log to `trades.json` and `bot_actions.json`
- [ ] **Per-symbol entry cooldown** — don't re-enter a symbol within 24 hours of closing it;
      enforced via `last_action_time` in `bot_state.json`
...
- [ ] **Market hours check** — no order submission outside 9:30–16:00 ET Mon–Fri;
      position review can still run (compute verdicts) but no orders are placed
- [ ] **Max daily loss** — if account equity drops more than `MAX_DAILY_LOSS_PCT` (default 2%)
...
- [ ] **Max concurrent positions** — already in `can_trade()`; verify it's enforced in
      the auto-entry path (it is, since auto-entry calls `can_trade()`)
- [ ] **Per-symbol cooldown** — 24-hour cooldown after any close (auto or manual)
- [ ] **Scan sanity check** — if a scan returns 0 results or raises an exception,
...
- [ ] **Bot status widget on positions page** — shows: scheduler running/paused,
      last scan time per timeframe, next scheduled scan, today's auto-actions count,
      halted status
- [ ] **Auto-action log section** — reads `data/bot_actions.json`; shows last 50 actions
      with timestamp, action type, symbol, and result
- [ ] **Pause/resume toggle** — API endpoint + UI button to set `bot_state.halted = True/False`
...
```
# Autonomous bot
BOT_AUTONOMOUS=true              # master on/off switch
BOT_SCAN_WATCHLIST=sp500_top100  # which symbol list to scan for entries
BOT_MAX_DAILY_LOSS_PCT=2.0       # halt entries if account drops this % in a day
BOT_ENTRY_COOLDOWN_HOURS=24      # hours before re-entering a recently closed symbol
BOT_REVIEW_TIMEFRAMES=swing,long # which timeframes to auto-review positions on
```
...
strategies/auto_manager.py    — core decision loop
data/bot_state.json           — persistent state (gitignored)
data/bot_actions.json         — append-only action log (gitignored)
```

## Files modified

```
app.py          — start scheduler on startup; add /api/bot/status, /api/bot/pause endpoints
config.py       — add BOT_* config vars
bot.py          — minor: expose get_positions() result to auto_manager cleanly
static/js/positions.js  — add bot status widget + action log section
```
